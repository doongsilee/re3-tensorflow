...
def kernel_to_image(data, padsize=1, padval=0):
    ...
    min = tf.compat.v1.reduce_min(tf.reshape(data, (dataShape[0], -1)), reduction_indices=1)
    data = tf.transpose((tf.transpose(data, (1, 2, 3, 0)) - min), (3, 0, 1, 2))
    max = tf.compat.v1.reduce_max(tf.reshape(data, (dataShape[0], -1)), reduction_indices=1)
...
def conv_layer(input, num_filters, filter_size, stride=1, num_groups=1, padding='VALID', scope=None,
    ...
    if bias_initializer is None:
        bias_initializer = tf.zeros_initializer()
    shape = [int(filter_width), int(filter_height), int(input.get_shape().as_list()[3] / num_groups), int(num_filters)]
...
def Session():
    return tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=tf.compat.v1.GPUOptions(allow_growth=True),
